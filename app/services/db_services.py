...
# Extended doctor profiles: (id, name, specialization, experience_years)
DEFAULT_DOCTORS = [
    ("Dr. Sharma",   "Dr. Sharma",   "Cardiologist",        12),
    ("Dr. Patel",    "Dr. Patel",    "General Physician",    8),
    ("Dr. Mehta",    "Dr. Mehta",    "Neurologist",         10),
    ("Dr. Khan",     "Dr. Khan",     "Emergency Medicine",  15),
    ("Dr. Iyer",     "Dr. Iyer",     "Pulmonologist",        9),
    ("Dr. Rao",      "Dr. Rao",      "Orthopedic Surgeon",  11),
]
...
def init_db():
    ...
    if existing == 0:
        ...
        for doc_id, name, spec, exp in DEFAULT_DOCTORS:
            c.execute(
                "INSERT OR IGNORE INTO doctors VALUES (?, ?, ?, ?, 0, NULL)",
                (doc_id, name, spec, exp)
            )
...
def discharge_patient(patient_id: str):
    ...
    if row:
        ...
        c.execute(
            "UPDATE patients SET status='Discharged', assigned_bed=NULL, assigned_doctor=NULL WHERE patient_id=?",
            (patient_id,)
        )
    conn.commit()
    conn.close()
...
def get_all_doctors() -> list:
    """Return full doctor profiles including specialization & experience."""
    conn = get_connection()
    c = conn.cursor()
...
def assign_specific_doctor(patient_id: str, doctor_id: str) -> bool:
    """Assign a specific doctor by ID to a patient. Returns True on success."""
    with _lock:
        conn = get_connection()
        c = conn.cursor()
        # Check doctor is still free
        doc = c.execute(
            "SELECT busy FROM doctors WHERE doctor_id=?", (doctor_id,)
        ).fetchone()
        if not doc or doc["busy"] == 1:
            conn.close()
            return False
